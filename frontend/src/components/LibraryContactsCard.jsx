...
export default function LibraryContactsCard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setContacts(await apiRequest('/api/library-contacts'));
    } catch (err) {
  ...
  const remove = async (id) => {
    try {
      await apiRequest(`/api/library-contacts/${id}`, { method: 'DELETE' });
      load();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div>
      <p className="text-xs text-ink-soft mb-3">Librarian WhatsApp numbers. No login needed — they just receive the daily AI-generated due/overdue books digest.</p>
      ...
      {loading ? (
        ...
        <div className="space-y-2 mb-3">
          {contacts.map((c) => (
            <div key={c.id} className="flex items-center justify-between p-2.5 rounded-lg bg-cream-deep/30">
              <div>
                ...
                <div className="text-xs text-ink-soft">{c.whatsapp_number}</div>
              </div>
          ...
          {contacts.length === 0 && <p className="text-sm text-ink-soft">No library contacts added yet.</p>}
        </div>
      )}
