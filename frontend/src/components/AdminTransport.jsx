...
const GPS_VENDORS = [
  { value: 'generic_poll', label: 'Demo / Reference (simulated)' },
  { value: 'trackmybus', label: 'TrackMyBus (not yet integrated)' },
  { value: 'traxroot', label: 'Traxroot (not yet integrated)' },
];
...
export default function AdminTransport() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selected && <BusPanel bus={selected} location={locations[selected.id]} onClose={() => setSelected(null)} />}
...
function BusPanel({ bus, location, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-cream rounded-2xl border border-cream-deep w-full max-w-lg shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mx-5 mb-3 text-xs text-ink-soft rounded-lg bg-cream-deep/50 px-3 py-2">
          Only "Demo / Reference" actually reports a moving location right now — other vendors need real API credentials before they'll show real positions.
        </p>
